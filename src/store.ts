 ...
 * to JSON files in ~/.kiro-to-im/data/.
 */
...
function writeJson(filePath: string, data: unknown): void { atomicWrite(filePath, JSON.stringify(data, null, 2)); }
...
export class JsonFileStore implements BridgeStore {
  ...
  private loadAll(): void {
    const sessions = readJson<Record<string, BridgeSession>>(path.join(DATA_DIR, 'sessions.json'), {});
    for (const [id, s] of Object.entries(sessions)) this.sessions.set(id, s);
    const bindings = readJson<Record<string, ChannelBinding>>(path.join(DATA_DIR, 'bindings.json'), {});
    for (const [key, b] of Object.entries(bindings)) this.bindings.set(key, b);
    const perms = readJson<Record<string, PermissionLinkRecord>>(path.join(DATA_DIR, 'permissions.json'), {});
    for (const [id, p] of Object.entries(perms)) this.permissionLinks.set(id, p);
    const offsets = readJson<Record<string, string>>(path.join(DATA_DIR, 'offsets.json'), {});
    for (const [k, v] of Object.entries(offsets)) this.offsets.set(k, v);
    const dedup = readJson<Record<string, number>>(path.join(DATA_DIR, 'dedup.json'), {});
    for (const [k, v] of Object.entries(dedup)) this.dedupKeys.set(k, v);
  ...
  getChannelBinding(channelType: string, chatId: string): ChannelBinding | null { return this.bindings.get(`${channelType}:${chatId}`) ?? null; }

  upsertChannelBinding(data: UpsertChannelBindingInput): ChannelBinding {
    const key = `${data.channelType}:${data.chatId}`;
    const existing = this.bindings.get(key);
    if (existing) {
      const updated: ChannelBinding = { ...existing, codepilotSessionId: data.codepilotSessionId, workingDirectory: data.workingDirectory, model: data.model, updatedAt: now() };
      this.bindings.set(key, updated);
      this.persistBindings();
      return updated;
    }
    const binding: ChannelBinding = {
      id: uuid(), channelType: data.channelType, chatId: data.chatId, codepilotSessionId: data.codepilotSessionId,
      sdkSessionId: '', workingDirectory: data.workingDirectory, model: data.model,
  ...
  updateChannelBinding(id: string, updates: Partial<ChannelBinding>): void {
    for (const [key, b] of this.bindings) {
      if (b.id === id) { this.bindings.set(key, { ...b, ...updates, updatedAt: now() }); this.persistBindings(); break; }
    }
  }
  ...
  createSession(_name: string, model: string, systemPrompt?: string, cwd?: string, _mode?: string): BridgeSession {
    const session: BridgeSession = { id: uuid(), working_directory: cwd || this.settings.get('bridge_default_work_dir') || process.cwd(), model, system_prompt: systemPrompt };
  ...
  getMessages(sessionId: string, opts?: { limit?: number }): { messages: BridgeMessage[] } {
    const msgs = this.loadMessages(sessionId);
    return opts?.limit && opts.limit > 0 ? { messages: msgs.slice(-opts.limit) } : { messages: [...msgs] };
  }

  acquireSessionLock(sessionId: string, lockId: string, owner: string, ttlSecs: number): boolean {
    const existing = this.locks.get(sessionId);
    if (existing && existing.expiresAt > Date.now() && existing.lockId !== lockId) return false;
  ...
  renewSessionLock(sessionId: string, lockId: string, ttlSecs: number): void {
    ...
    if (lock && lock.lockId === lockId) lock.expiresAt = Date.now() + ttlSecs * 1000;
  }

  releaseSessionLock(sessionId: string, lockId: string): void {
    const lock = this.locks.get(sessionId);
    if (lock && lock.lockId === lockId) this.locks.delete(sessionId);
  }

  setSessionRuntimeStatus(_sessionId: string, _status: string): void { /* no-op */ }

  updateSdkSessionId(sessionId: string, sdkSessionId: string): void {
    const s = this.sessions.get(sessionId);
    if (s) { (s as unknown as Record<string, unknown>)['sdk_session_id'] = sdkSessionId; this.persistSessions(); }
    for (const [key, b] of this.bindings) {
      if (b.codepilotSessionId === sessionId) this.bindings.set(key, { ...b, sdkSessionId, updatedAt: now() });
    }
    this.persistBindings();
  }
  ...
  syncSdkTasks(_sessionId: string, _todos: unknown): void { /* no-op */ }
  getProvider(_id: string): BridgeApiProvider | undefined { return undefined; }
  ...
  checkDedup(key: string): boolean {
    ...
    if (Date.now() - ts > 5 * 60 * 1000) { this.dedupKeys.delete(key); return false; }
    return true;
  }
  ...
  cleanupExpiredDedup(): void {
    ...
    let changed = false;
    for (const [key, ts] of this.dedupKeys) { if (ts < cutoff) { this.dedupKeys.delete(key); changed = true; } }
    if (changed) this.persistDedup();
  }

  insertOutboundRef(_ref: OutboundRefInput): void { /* no-op */ }

  insertPermissionLink(link: PermissionLinkInput): void {
    this.permissionLinks.set(link.permissionRequestId, {
      permissionRequestId: link.permissionRequestId, chatId: link.chatId, messageId: link.messageId, resolved: false, suggestions: link.suggestions,
    });
    this.persistPermissions();
  }

  getPermissionLink(permissionRequestId: string): PermissionLinkRecord | null { return this.permissionLinks.get(permissionRequestId) ?? null; }
  ...
  getChannelOffset(key: string): string { return this.offsets.get(key) ?? '0'; }
