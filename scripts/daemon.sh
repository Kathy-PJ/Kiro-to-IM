...
STATUS_FILE="$KTI_HOME/runtime/status.json"
LOG_FILE="$KTI_HOME/logs/bridge.log"

# ── Common helpers ──

ensure_dirs() { mkdir -p "$KTI_HOME"/{data,logs,runtime,data/messages}; }
...
ensure_built() {
  ...
  if [ ! -f "$SKILL_DIR/dist/daemon.mjs" ]; then
    ...
    # v2: no claude-to-im dependency, all source is in src/
  fi
